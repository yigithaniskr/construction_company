...
const ProjectDetailsÇeliktepeKentsel = () => {
  const { t } = useTranslation(); // i18n çeviri fonksiyonu kullanımı
  const [lightboxIndex, setLightboxIndex] = useState(null);

  const projectInfo = {
    name: t("project_celiktepe_kentsel.name"),
    location: t("project_celiktepe_kentsel.location"),
    deliveryDate: t("project_celiktepe_kentsel.deliveryDate"),
    area: t("project_celiktepe_kentsel.area"),
    description: t("project_celiktepe_kentsel.description"),
  };

  const projectImages = [
    "/Çeliktepe_kentsel2.png",
    "/Çeliktepe_kentsel1.png",
  ];
  ...
  return (
    <HelmetProvider>
      <Helmet>
        <title>{t("project_celiktepe_kentsel.page_title")}</title>
      </Helmet>
      <Header
      ...
      <div className="project-container">
        <div className="project-header">
          ...
          <p>
            <strong>{t("project_celiktepe_kentsel.location_label")}:</strong>{" "}
            {projectInfo.location}
          </p>
          <p>
            <strong>{t("project_celiktepe_kentsel.delivery_label")}:</strong>{" "}
            {projectInfo.deliveryDate}
          </p>
          <p>
            <strong>{t("project_celiktepe_kentsel.area_label")}:</strong>{" "}
            {projectInfo.area}
          </p>
...
export default ProjectDetailsÇeliktepeKentsel;